...
int main()
{
    ...
    { //child process: collect the recived data from the named pipe and display in the terminal
       const char * myfifo = "/tmp/BtoA";  // name the pipe, that is why we call it named pipe, you can access this as long as you have the name
        ...
        while (fd < 0)   //(1)//(c)// we need to wait the other to create the FIFO, a.k.a., the named pipe. It may have not been created yet if you run this code first.
    ...
    {//parent process: collect the user input from the terminal and send via the pipe
        ssize_t nread;
        int fds;
        const char * myfifos = "/tmp/AtoB"; //(2)// name the pipe, should be different
